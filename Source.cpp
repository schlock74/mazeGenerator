...
struct cell
{
	...
	{
		//looks in each direction to see if there are adjacent cells that haven't been visited yet
		//if they have not been visited add them to the list of unvisited cells
		if ((posX + 1) > (width - 1))
		{
		}
		...
		//if there are neighboring cells return TRUE else return FALSE
...
void initializeMaze(int width, int height, cell maze[][10])			//initializes each cell in the grid
{
...
int main()
{
	...
	{		
		if (grid[curPosX][curPosY].checkNeighbor(width, height, grid, unvisitedCell) == true)
		{
			...
			//clear out the list of adjacent cells
			unvisitedCell.clear();	
		}
		else if (grid[curPosX][curPosY].checkNeighbor(width, height, grid, unvisitedCell) == false)
		{
			//while the stack is not empty and top of stack has no unvisited neighbors 
			do 
			{
				//set top to current cell and then pop stack
